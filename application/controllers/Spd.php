...
class Spd extends CI_Controller {
	...
	public function proses_create2(){
		...
		$timedeparture = $this->input->post('dept_time');
		$expectdate = $this->input->post('return_time');
	...
	public function proses_create(){
		...
		$pickup 	= $this->input->post('loc_pickup');
		$lat1 		= $this->input->post('lat1');
		$lng1 		= $this->input->post('lng1');
		$destination = $this->input->post('loc_destination');
		$lat2 		= $this->input->post('lat2');
		$lng2 		= $this->input->post('lng2');
		...
		$status 	= $this->input->post('status');
		$cost_center = $this->input->post('cost_center');
		$cost_approval = $this->input->post('cost_approval');
		...
		$place = [
			...
			'jarak'=> $distance,
			'timestamp' => time(),
			'driver_pickup' => $this->input->post('departure_driver'),
			'mobil_pickup' => $this->input->post('departure_mobil'),
			'driver_destination' => $this->input->post('return_driver'),
			'mobil_destination' => $this->input->post('return_mobil')
		];
		...
		$spd = [
			...
			'id_bu' => $this->session->userdata('site'),
			'draft' => $save,
			'kendaraan_berangkat' => json_encode($this->input->post('dept_kendaraan')),
			'kendaraan_pulang' => json_encode($this->input->post('return_kendaraan')),
			'operasional_out' => $this->cek_number($this->input->post('dept_kendaraan')),
			'operasional_in' => $this->cek_number($this->input->post('return_kendaraan'))
		];
		...
		if ($save == 'submit') {
			...
			$this->db->insert('tbl_log_approval', $data);
		} else {
			$save = 0;
		}
	...
	private function cek_number($arr){
		foreach ($arr as $ar) {
			if ($ar == 1) {
				return 1;
			} else{
